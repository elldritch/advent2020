...
spec = do
  ...
  it "finds the fixed point of layout steps" $ do
    length (filter (== Occupied) $ toList $ grid $ fixed (step adjacent 4) exampleGrid) `shouldBe` 37

  it "computes visible seats" $ do
    (parse exampleInput2 >>= (\g -> return $ firstVisibleSeat g (3, 4))) `shouldBe'` replicate 8 Occupied
    (parse exampleInput3 >>= (\g -> return $ firstVisibleSeat g (1, 1))) `shouldBe'` [Empty]
    (parse exampleInput4 >>= (\g -> return $ firstVisibleSeat g (3, 3))) `shouldBe'` []

  it "finds fixed point layouts for people who care about visible seats" $ do
    length (filter (== Occupied) $ toList $ grid $ fixed (step firstVisibleSeat 5) exampleGrid) `shouldBe` 26
