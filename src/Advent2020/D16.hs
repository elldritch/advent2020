...
run :: ((Rules, Ticket, NonEmpty Ticket) -> a) -> Text -> Either Text a
run = simpleRun' parse return

part1 :: (Rules, Ticket, NonEmpty Ticket) -> Integer
part1 (rules, _, tickets) = foldr sumInvalidValues 0 tickets
  where
...
part2 :: (Rules, Ticket, NonEmpty Ticket) -> Integer
part2 (rules, ticket, tickets) = product $ catMaybes values
  where
